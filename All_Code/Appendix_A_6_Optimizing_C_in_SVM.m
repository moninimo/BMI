%%CODE TO FIND OPTIMAL C GIVEN sigma found before AND NO PCA-LDA
clc;clear;close all;
load 'monkeydata_training.mat'
dt=80 %VARY THIS PARAMETER TO GETR PLOTS FOR DIFFERENT DT
max_iter=2;
T_vec=320:20:560;
c_vec=[0.01 0.1 1 5 10];
s2=0.1;
if dt==80
    s2=0.07;
end
acc=zeros([length(T_vec) length(c_vec)+1 max_iter]);
mse_if_last=zeros([length(T_vec) length(c_vec)+1 max_iter]);
mse_up_to=zeros([length(c_vec)+1 max_iter]);
...
for iter=1:max_iter
    ...
    for ind_T=1:length(T_vec)
        ...
        for ind_c=1:length(c_vec)
            c=c_vec(ind_c);
            [pred]=do_SVM(Xt,mF(t<=T,:),s2,c);
            acc(ind_T,ind_c+1,iter)=mean(pred==l_test,2);
            er_at_time=0;
            er_if_last=0;
            for tr=1:length(pred)
                er_at_time=er_at_time+...
            ...
            mse_if_last(ind_T,ind_c+1,iter)=mse_up_to(ind_c+1,iter)+er_if_last;
            mse_up_to(ind_c+1,iter)=mse_up_to(ind_c+1,iter)+er_at_time;
        end
